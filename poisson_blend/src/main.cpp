...
bool isVec3Equal(vec3 v1, vec3 v2) {
	return isFloatEqual(v1.x, v2.x) && isFloatEqual(v1.y, v2.y) && isFloatEqual(v1.z, v2.z);
}


void normalizeBlackWhiteMask(ImageData& mask) {
	for (auto& pixel : mask.data) {
		if (isVec3Equal(pixel, vec3(0, 0, 0))) {
			pixel.x = 1.0;
		}
		else {
			pixel.x = 0.0;
...
void addPadding(ImageData& img, int padding_size) {
	
	int new_width = img.width + 2 * padding_size;
	int new_height = img.height + 2 * padding_size;
	std::vector<vec3> newData(new_width * new_height, vec3(0, 0, 0));

	for (int x = 0; x < img.width; x++) {
		for (int y = 0; y < img.height; y++) {
			// int newX = x + 1;
			// int newY = y + 1;
			newData[(y + padding_size) * new_width + x + padding_size] = img.data[y * img.width + x];
		}
	}
...
void removePadding(ImageData& img, int padding_size) {
...
void saveImage(const char* filename, ImageData& imgData) {
	std::vector<unsigned char> outImage;

		// first, output the original image to outImage
		for (unsigned int i = 0; i < imgData.data.size(); ++i) {
			vec3 v = imgData.data[i];
			outImage.push_back((unsigned char)(v[0] * 255.0f));
			outImage.push_back((unsigned char)(v[1]* 255.0f));
			outImage.push_back((unsigned char)(v[2] * 255.0f));
			outImage.push_back(255);
		}
	lodepng::encode(filename, outImage, imgData.width, imgData.height);
}
...
int main(int argc, char *argv[]) {
	...
	// normalize maskImage so that it support black - white images.
	normalizeBlackWhiteMask(maskImage);

	// // save normalized mask
	// const char* normalizedMaskFile = "normalized_mask.png";
	// saveImage(normalizedMaskFile, maskImage);

	// // add padding to target image
	addPadding(targetImage, paddingSize);
	// const char* paddedFile = "padded_target_image.png";
	// std::cout << targetImage.width << " " << targetImage.height << std::endl;
	// saveImage(paddedFile, targetImage);
	...
	*/
	...
	// finally, we output the poisson blended image.
	{
		...
		// remove the padding from out image
		std::vector<unsigned char> outImageNoPadding(4 * (targetImage.width - 2 * paddingSize) * (targetImage.height - 2 * paddingSize), '0');

		for (unsigned int y = paddingSize; y < targetImage.height - paddingSize; ++y) {
			for (unsigned int x = paddingSize; x < targetImage.width - paddingSize; ++x) {
				// std::cout << x << " " << y << std::endl;
				outImageNoPadding[4 * ((y - paddingSize) * (targetImage.width - 2 * paddingSize) + x - paddingSize) + 0] = outImage[4 * (y * targetImage.width + x) + 0];
				outImageNoPadding[4 * ((y - paddingSize) * (targetImage.width - 2 * paddingSize) + x - paddingSize) + 1] = outImage[4 * (y * targetImage.width + x) + 1];
				outImageNoPadding[4 * ((y - paddingSize) * (targetImage.width - 2 * paddingSize) + x - paddingSize) + 2] = outImage[4 * (y * targetImage.width + x) + 2];
				outImageNoPadding[4 * ((y - paddingSize) * (targetImage.width - 2 * paddingSize) + x - paddingSize) + 3] = outImage[4 * (y * targetImage.width + x) + 3];
			}
		}

		// std::cout <<  targetImage.width - 2 * paddingSize << " " << targetImage.height - 2 * paddingSize << std::endl;
		lodepng::encode(outputFile, outImageNoPadding, targetImage.width - 2 * paddingSize, targetImage.height - 2 * paddingSize);
	}
}
